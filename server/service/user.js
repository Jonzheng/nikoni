...
exports = module.exports = {
  ...
  queryNews: async (ctx) => {
    ...
    resHeart = resHeart[0]
    let resComment = await mysql.raw(`select tur.openid,tur.nick_name,tur.show_name,tur.motto,tur.avatar_url,tc.record_id,tc.file_id,tc.content, unix_timestamp(tc.c_date) as times, tc.c_date, 'comment' as type from t_comment tc LEFT JOIN t_user tur on (tc.user_id = tur.openid) LEFT JOIN t_comment tc2 on (tc.re_id = tc2.id) WHERE tc.status = 1 and (tc2.user_id = ? or (tc.master_id = ? and tc.user_id != ?)) ORDER BY tc.c_date desc limit 0,50`, [openid, openid, openid]);
