...
exports = module.exports = {
  ...
  queryFollow: async (ctx) => {
    ...
    let data = await mysql.raw('select tur.openid,tur.nick_name,tur.show_name,tur.motto,tur.avatar_url,tf.c_date FROM t_follow tf LEFT JOIN t_user tur on (tf.follow_id = tur.openid) where tf.status = 1 and tf.openid = ? limit ?,?', [openid, offset, pageSize]);
    ctx.body = data[0];
  },
  queryFans: async (ctx) => {
    let body = ctx.request.body
    ...
    pageSize = pageSize ? pageSize : 10
    let offset = (pageNo - 1) * pageSize
    let data = await mysql.raw('select tur.openid,tur.nick_name,tur.show_name,tur.motto,tur.avatar_url,tf.c_date FROM t_follow tf LEFT JOIN t_user tur on (tf.openid = tur.openid) where tf.status = 1 and tf.follow_id = ? limit ?,?', [openid, offset, pageSize]);
    ctx.body = data[0];
  },
  ...
  queryNews: async (ctx) => {
    ...
    let resFans = await mysql.raw(`select tur.openid,tur.nick_name,tur.show_name,tur.motto,tur.avatar_url,tf.c_date,unix_timestamp(tf.c_date) as times, 'fans' as type FROM t_follow tf LEFT JOIN t_user tur on (tf.openid = tur.openid) where tf.status = 1 and tf.follow_id = ? ORDER BY tf.c_date desc limit 0,30`, openid);
    resFans = resFans[0]
